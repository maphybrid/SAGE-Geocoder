...
def TableExtract(SourceTable, Workspace, ID, Address, City, State, Zip):
    AddressList = []
    if ID==0:
        arcpy.TableToTable_conversion(SourceTable, Workspace, "Addresses.dbf", None)
        Dbase = Workspace+"\\Addresses.dbf"
        AddressCheck(Dbase,Workspace, Address)
...
#---environment settings--------------------------------------------------------
    ...
    Messages("s")
#---address list----------------------------------------------------------------
    addressList = TableExtract(SourceTable, Workspace,count,Address1,City,State,Zip)
#---geocoder--------------------------------------------------------------------
    Coder, CallID = GeocodeCaller(CoderID)
    arcpy.AddMessage("")
    Messages(i)
    CoordList = USGeocode(Workspace,addressList,Coder,CallID,count)
    Messages("s")
    i=i+1
#--coordinate storage-----------------------------------------------------------
    Messages(i)
